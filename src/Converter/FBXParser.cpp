...
void FBXParser::_parseAllMeshesAsOne(const aiScene* scene) {
    ...
    for (unsigned int mi = 0; mi < scene->mNumMeshes; mi++) {
        ...
        // Material slot
        mesh.materialSlots.push_back("materials/" + std::to_string(m->mMaterialIndex) + ".aymat");
...
void FBXParser::_extractMaterialTextures(const aiMaterial* mat, MaterialData& material) {
    ...
    for (const auto& [texType, paramName] : textureTypes) {
        ...
        if (mat->GetTexture(mappedType, 0, &texPath) == AI_SUCCESS) {
            ...
            // 去掉绝对路径前缀（保留相对目录）
            // 例如：D:\Models\tex\skin.png -> tex/skin.png
            // 或：tex/skin.png -> tex/skin.png
            ...
            // 构建纹理资源路径（与 FBXConverter section 4b/7 保持一致）
            // 子目录用下划线替代斜杠
            ...
            size_t lastSep = textureName.find_last_of("/\\");
            if (lastSep != std::string::npos) {
                textureName = textureName.substr(0, lastSep) + "_" + textureName.substr(lastSep + 1);
            }
            ...
            param.texturePath = "textures/" + textureName + ".aytex";
...
void FBXParser::_parseMaterial(const void* aiMatPtr, size_t index) {
    ...
    material.shader = "shaders/pbr.phoskia";
